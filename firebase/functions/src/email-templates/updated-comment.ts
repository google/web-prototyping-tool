...
export const createHtmlForResolutionStateChange = async (
  data: cd.IResolvedCommentEmailAlertConfig,
  targetId: string
) => {
  const { projectId, comments, resolved } = data;
  const resolvedText = resolved ? 'resolved' : 're-opened';
  const headingText = `A thread you are included in has been ${resolvedText} on ${consts.APP_NAME}`;
  ...
  for (const comment of comments) {
    commentHtml += await templateUtils.createComment(comment);
  }
